...
<div class="form-group row {{ $errors->has('permission_id') ? ' has-error' : '' }}">
    {{ Form::label('permission_id', 'Permisos *', ['class' => 'control-label col-md-2 col-sm-2 col-xs-10']) }}

    <div class="col-md-6 col-sm-6 col-xs-8">
        {{ Form::select('permission_id[]', $permissions, $permission_id, [
...
@section('js-scripts')
    <script>
      ...
      function refreshSelects(){
        $("#permission_id").css({width:'100%'}).select2({
            allowClear: true,
            placeholder: {
                id: '',
